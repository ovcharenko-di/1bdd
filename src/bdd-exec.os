...
//	будет получено "d:\myproject\file.txt"
//
...
Функция ПолучитьПутьФайлаСУчетомПеременныхКонтекста(Знач Путь) Экспорт
	...
	Лог.Отладка("ПолучитьПутьФайлаСУчетомПеременныхКонтекста %1", Рез);
	
	// ранний возврат для абсолютных путей в Linux
	Если СтрНачинаетсяС(Рез, "/") Тогда
		Лог.Отладка("Передан абсолютный путь, подмена из переменных контекста не выполняется");
		Возврат Рез;
	КонецЕсли;

	ЧастиПути = СтрРазделить(СтрЗаменить(Путь, "\", "/"), "/", Ложь);
	Если ЗначениеЗаполнено(ЧастиПути) Тогда
		...
		Если ЗначениеИзКонтекста <> Неопределено Тогда
			...
			ЧастиПути.Удалить(0);
			ЧастиПути.Вставить(0, ИмяКорня);
		КонецЕсли;
		Рез = СтрСоединить(ЧастиПути, ПолучитьРазделительПути());
	КонецЕсли;
