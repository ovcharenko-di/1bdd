...
// Я копирую файл проекта "file.txt" в каталог ".."
Процедура ЯКопируюФайлПроектаВКаталог(Знач ПутьФайла, Знач КаталогКуда) Экспорт
	...
	Лог.Отладка("ПутьФайла %1", ПутьФайла);
	Лог.Отладка("КаталогКуда %1", КаталогКуда);
	Лог.Отладка("ИмяФайлаПриемника %1", ИмяФайлаПриемника);
...
Функция ФайлИлиКаталогСуществует(Знач ПутьФайла, ПолныйПуть, Знач ЭтоКаталог = Ложь, Знач ИсходныйКаталог = "")
	...
	Попытка
		...
		ЭтоПутьСРегуляркой = РегулярноеВыражение.Совпадает(ПолныйПуть);

		Маска = Неопределено;
		Если ЭтоПутьСРегуляркой Тогда
			Файл = Новый Файл(ПолныйПуть);
			// если есть регулярка, то считаем, что полный путь делится на путь и маску
			ПолныйПуть = Файл.Путь;
			Маска = Файл.Имя;
			Файл = Неопределено;
		КонецЕсли;

		МассивФайлов = НайтиФайлы(ПолныйПуть, Маска);
